...
2026-08-15時点のリポジトリには、プロダクト文書とUI正本の `デザインシート.svg` はあるが、実行可能な拡張機能のソース、`package.json`、依存関係ロックファイル、ビルドスクリプトはまだない。旧企画PDFは情報が古いため削除済みである。したがって、現時点ではBookmationをインストール、起動、ビルド、テストできない。
...
- 同名MAINを作成できず、同名SUBは別IDで表示でき、同じブックマークへ同じタグIDを二重付与しない。
- 共通AI検索がタグ／ブックマークの無順位候補を複数返し、AI利用不可時は文字列候補であると明示する。
- リスト／グリッドだけを切り替えられ、MAINは常時、SUBはキーボードでも展開できる。
- ホームには最近追加が表示され、全画面タグ一覧の選択で対象一覧へ移動できる。
