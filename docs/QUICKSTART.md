...
2026-08-15時点のリポジトリには、プロダクト文書、企画PDF、UI正本の `デザインシート.svg` はあるが、実行可能な拡張機能のソース、`package.json`、依存関係ロックファイル、ビルドスクリプトはまだない。したがって、現時点ではBookmationをインストール、起動、ビルド、テストできない。
