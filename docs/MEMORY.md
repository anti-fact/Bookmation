...
- 分類は親の「カテゴリ」と子の「タグ」を区別する。永続enumは `CATEGORY` / `TAG` だが、UIに英語enumや旧メイン／サブの名称を表示しない。各Tagは物理的に存在する1件の親Categoryに所属し、active Tagはactive親を必須とする。tombstone Tagはdeleted親を参照できるが、親Categoryを先に復元しなければ戻せず、子Tag tombstoneが残る親は物理回収しない。UIではカテゴリリボンの配下にactiveな子タグを表示する。
...
- カテゴリとタグはどちらも論理削除中を含めてLabel Normalizer v1の正規化名で一意とし、Tagは親Categoryをまたいでglobal uniqueにする。同名作成は拒否し、既存項目の選択、削除済み元IDの復元、または別名を促す。v1はproject-vendored Unicode 15.1.0データに固定し、runtime ICUへ依存しない。物理回収後だけ名前を再利用できる。
...
- 破壊的な移行には事前確認とバックアップを用意する。Bookmark、カテゴリ、タグの削除は確認画面を挟まず論理削除し、直後のundoと復元経路を必ず用意する。有効な子タグが残るカテゴリの削除はblockし、cascade deleteしない。
...
| 分類名の一意性 | カテゴリ名とタグ名はそれぞれ論理削除中を含めて正規化後にglobal uniqueとする。Tagの親カテゴリが異なっても同名の別IDは作らず、削除済みなら元IDを復元する |
