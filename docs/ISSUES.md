...
| ISSUE-003 | Open | P0 | 弁当カードの大小に意味が定まっていない | 決定的パターン、ピン留め、画像比率のいずれかをユーザーテストで選び、DOM順を保証する |
| ISSUE-004 | Open | P0 | AI サブタグ細分化5段階の上限は仮値である | 代表データセットで過剰生成率・修正率・候補再利用率を測り、段階名と新規サブタグ上限を確定する |
| ISSUE-005 | Open | P0 | 同名タグを許可するため、候補の見分けと手動統合の UX が必要である | ID、MAIN / SUB、作成元、関連件数の表示方法と、ユーザーが明示実行する統合・復元の仕様を決める |
...
| ISSUE-014 | Open | P0 | 自然言語のタグ検索とブックマーク検索のランキング・応答時間・AI 非対応時の縮退動作が未確定である | 両結果の入力と表示分離、最大候補数、理由表示、応答時間、AI 利用不可時のテキスト検索をプロトタイプで決める |
...
| ID | 状態 | 決定 | 根拠 |
| --- | --- | --- | --- |
| ISSUE-D01 | Decided | PDF の左フォルダツリーではなく、右タグサイドバーを採用する | 今回の依頼が PDF より新しく具体的である |
...
| ISSUE-D07 | Decided | MAIN / SUB はどちらも複数割当を許可し、同名タグレコードも許可する | 最新の利用者依頼の「全てのタグは重複あり」を明文化した |
| ISSUE-D08 | Decided | UI 実装基盤は Plasmo（React ベース）+ Tailwind CSS とする | 最新の利用者依頼 |
| ISSUE-D09 | Decided | ポップアップの2ボタン、2ショートカット、URL 指定保存、最近追加ホーム、自然言語の2種検索を P0 に含める | 最新の利用者依頼 |
