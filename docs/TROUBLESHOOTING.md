...
| タグが増えすぎる / 同名タグを見分けられない | [分類](#タグが増えすぎるまたは同名タグを見分けられない) |
...
- 利用者定義の一致候補を最優先する。
- 新規タグ数、文字数、禁止値を決定的な後処理で検証し、細分化スライダーの上限を超えた候補を拒否する。
...
- 共通フォームの結果を片方のentity typeへ誤って絞っている。
...
1. 両一覧の検索ボックスが同じqueryを使い、1回の送信でLabel / Bookmarkの2集合が返るか確認する。
...
- 1つの入力と、上段の `カテゴリ・タグ`、下段の `ブックマーク` の結果グループを表示する。
...
1. 設定toggle、訪問回数閾値、archive日数、`history` / `notifications` 権限を確認する。
...
4. Reminder stateと `nextEligibleAt` を確認し、完全な履歴やURLを通常ログへ出さない。

**対処**

- 権限拒否時は設定をオフのままにし、権限を無断で再要求しない。
- alarmは起動時に冪等再登録する。sleep中に実行される正確な時刻を仮定しない。
- 通知の `保存` 前にBookmarkが作られていれば不具合として停止する。
- `lastVisitedAt=null` は自動archiveせず、ARCHIVEDは復元する。履歴削除やデータ初期化で直そうとしない。

## QR、Drive、標準Bookmark取込、右クリック保存が失敗する

**確認**

- QRはschemaVersion、byte数、checksum、preview結果を確認する。不明版を無理に取り込まない。
- Driveは接続アカウント、`drive.appdata` scope、token失効、未同期Outbox、conflictを確認する。tokenをログへ貼らない。
...
- QR破損、不明版、容量超過は書き込まず再生成を依頼する。
- Drive認証失効は明示的な再接続で直し、ローカル編集とOutboxを消さない。
