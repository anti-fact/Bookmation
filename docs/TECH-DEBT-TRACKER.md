...
| TD-005 | P1 | 分類・削除 | Unicode 15.1.0 vendored assetに基づくLabel Normalizer v1、active／tombstone親子、名前予約・一意性、削除operationが未実装 | runtime ICU差で同名判定が変わる、削除→同名作成→undoで衝突する、deleted親を先にGCしてTagを復元不能にする恐れがある | NFKC／White_Space／Default_Ignorable_Code_Point／CaseFolding C＋F assetとhashを固定し、runtime ICU非依存にする。tombstoneを含むunique index、親先行Tag restore、子Tag tombstone残存中の親GC拒否、3 entityのdeleteOperationId＋revision、UNDO_EXPIRED／UNDO_CONFLICTを実装する。親変更はISSUE-019決定後とする | 登録 |
...
| TD-010 | P1 | UI | sticky header、可変高タグ、親子一覧、管理モード、フルページ検索、無限scroll、back-to-topの大量件数性能が未検証 | scroll位置、focus、候補popup、side view、重複取得、描画性能が破綻し得る | デザインシート準拠prototypeを測定し、focus復帰、cursor・observer・virtualizationの性能予算を決める | 登録 |
...
| TD-017 | P0 | テスト | production UIのWeb harnessと実拡張Playwright E2Eがない | Unicode asset差、Tag親restore／GC、operation undo、AI snapshot、URL抑止、Drive snapshot／resolutionを人間が再現しにくく、Chrome固有不具合を見逃す恐れがある | [TESTING.md](TESTING.md) とTASK-013に沿い、Normalizer asset hash、親子tombstone、競合snapshot保持を含む決定的fixture、隔離profile、report／screenshot／trace、人間受入記録を実装する | 登録 |
