...
| TD-005 | P1 | 分類・削除 | Unicode 15.1.0 vendored assetに基づくLabel Normalizer v1、active／tombstone親子、名前予約・一意性、確認なしsoft-deleteが未実装 | runtime ICU差で同名判定が変わる、削除後に同名別IDが作られる、deleted親を子tombstoneより先にGCする恐れがある | NFKC／White_Space／Default_Ignorable_Code_Point／CaseFolding C＋F assetとhashを固定し、runtime ICU非依存にする。tombstoneを含むunique index、子Tag tombstone残存中の親GC拒否、3 entityの確認なしsoft-deleteを実装する。削除Undoのtoken／期限／復元経路は作らない。親変更はISSUE-019決定後とする | 登録 |
...
| TD-017 | P0 | テスト | production UIのWeb harnessと実拡張Playwright E2Eがない | Unicode asset差、親子tombstone／GC、削除Undo経路の誤混入、AI snapshot、URL抑止、Drive snapshot／resolutionを人間が再現しにくく、Chrome固有不具合を見逃す恐れがある | [TESTING.md](TESTING.md) とTASK-013に沿い、Normalizer asset hash、親子tombstone、確認なし削除とUndo経路なし、競合snapshot保持を含む決定的fixture、隔離profile、report／screenshot／trace、人間受入記録を実装する | 登録 |
