...
| TD-011 | P1 | 共有・同期 | QR容量超過時のCSV fallback、CSV安全化、checksumの限界、異親同名Tag解決、Drive競合snapshot／resolution／保持が未実装 | QRの切捨て、CSV数式注入、checksumを真正性保証と誤認する、Tagを誤parentへ再利用する、LWWや暗黙remapで更新・Label edgeを失う恐れがある | 同じ固定選択集合のQR／CSV export、encoder容量超過時のCSV誘導、CSV escaping／formula neutralization／秘密情報除外を実装する。checksumを破損検出に限定し、異親同名Tagは別名／skip／cancel再previewとする。Drive競合はimmutable syncSnapshotsと明示resolution planへ隔離し、open中GC禁止、解決後30日保持、暗黙Label／edge remap禁止を実装する | 登録 |
...
| TD-014 | P1 | 履歴・archive | 訪問日数の7／30／365日集計、期間変更時clear、URL別reset、旧回数設定migration、archive toggle権限gate・既定30日・履歴なしエラーが未実装 | 同日訪問を過大計上する、`いいえ` 後すぐ再通知する、旧回数を日数へ誤変換する、権限なしでarchiveする、履歴なしを無言skipする、復元不能につながる | 訪問日数既定null、期間別境界、同日重複排除、`countingResetAt`、旧設定の再設定要求、canonical URL単位SUPPRESSED、確認前保存禁止、autoArchiveEnabled既定OFF／history許可時だけON／取消時OFF、archiveAfterDays既定30、履歴なし項目別エラー／archive不可、最小archiveと復元をfixture化する | 登録 |
| TD-015 | P1 | インポート | 直上FolderだけをTag化する標準Bookmark取込、重複UI、途中再開が未実装 | 祖先／full path／AI Tagの混入、親Categoryの暗黙作成、分類乱立、二重取込、元データ変更の危険がある | 読取専用adapter、直上Folder→Tag解決、同名active Tag再利用、新規Tagの親Category選択、空／不正／tombstone競合、AI Jobなし、preview、Import Job、元tree不変契約を実装する | 登録 |
...
| TD-017 | P0 | テスト | production UIのWeb harnessと実拡張Playwright E2Eがない | Tag編集のCategory候補／side view／親変更fan-out、Category使用状況、cascade削除／再分類、削除Undo経路の誤混入、AI snapshot、URL抑止、archive権限gate／履歴なし、QR容量超過CSV誘導、右クリック設定と実menuのずれ、Drive snapshot／resolutionを人間が再現しにくく、Chrome固有不具合を見逃す恐れがある | [TESTING.md](TESTING.md) とTASK-013に沿い、Tag／親expected revision、Category自動導出、side view draft、全参照Bookmarkの原子的更新／AI再分類なし、Category警告付きcascadeとPENDING／NEEDS_REVIEW再分類、Undo経路なし、archive許可／拒否／取消／履歴なし、QR／CSV、右クリックON／OFF・再起動・遅延clickを含む決定的fixture、隔離profile、report／screenshot／trace、人間受入記録を実装する | 登録 |
