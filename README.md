...
各コマンドはリポジトリのルートで `pnpm <コマンド名>` として実行する。

| コマンド         | 実行内容                                                                                 | 主な用途                                                                                                                      |
| ---------------- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `dev`            | Plasmoの開発モードを起動し、変更を監視しながら開発用のChrome拡張機能をビルドする         | 実際のChrome拡張機能として動作確認する。生成された開発用extensionをChromeへ「パッケージ化されていない拡張機能」として読み込む |
| `build`          | Plasmoで配布用のChrome拡張機能をビルドする                                               | 本番向け成果物が生成できるか確認する                                                                                          |
| `lint`           | ESLintをリポジトリ全体へ実行する                                                         | コード規約違反や静的解析上の問題を検出する                                                                                    |
| `typecheck`      | TypeScriptコンパイラを出力なしで実行する                                                 | JavaScriptを生成せず、型エラーだけを検出する                                                                                  |
| `test`           | Vitestの全テストを1回実行して終了する                                                    | 変更後の自動テストと回帰テストをまとめて確認する                                                                              |
| `test:idb`       | IndexedDBのローカルデータ層テストだけを1回実行して終了する                               | IndexedDBのスキーマ、保存、更新、削除などを重点的に確認する                                                                   |
| `test:idb:watch` | IndexedDBのローカルデータ層テストをwatchモードで起動し、関連ファイルの変更時に再実行する | IndexedDB周辺を実装しながら短いサイクルで確認する                                                                             |
| `ui:preview`     | Viteでfake Adapterとfixtureを使う通常WebページのUIプレビューを起動する                   | Chromeへ拡張機能を読み込まず、共通UIの表示や操作を素早く確認する                                                              |
| `ui:build`       | ViteでUIプレビュー用Webページを本番ビルドする                                            | fixtureを含むUIプレビューが静的にビルドできるか確認する                                                                       |

`ui:preview` は共有レイアウトとfeature UIを確認するためのWeb fixtureであり、Chrome権限、Service Worker、拡張機能popup、実際の拡張機能storageは検証しない。これらを含む動作確認には `dev` または `build` で生成した拡張機能をChromeへ読み込む。
