...
Bookmation は、Chrome 標準ブックマークとは別の専用領域へ Web ページを保存し、MAIN / SUB タグと検索で見つけ直す Chrome 拡張機能である。
...
- ブックマーク一覧はリスト／グリッドを切り替え、追従ヘッダーでキーワード検索、件数確認、AI検索を行う。
- 各ブックマークは MAIN を常時表示し、SUB をクリック／キーボードで展開できる。各項目の編集モーダルで名前、URL、タグ、削除を操作できる。
- タグ一覧は全画面で開き、追従するタグ専用キーワード検索、AI検索、閉じる、トップへ戻る操作を持つ。
- 一覧は下端へ近づくたび次ページを自動読込する。
- 共通 AI 検索は1つの自然言語フォームからタグとブックマークを探し、無順位の候補集合を種類別に複数表示する。

## タグと AI

- MAIN / SUB は親子階層のない平坦なタグで、1件にそれぞれ複数付与できる。
- MAIN はユーザーだけが作成し、正規化した同名 MAIN は重複させない。
- SUB は同名の別 ID を許す。AI はユーザー定義 SUB を優先し、不足時だけ作る。
- 設定モーダルの5段階スライダーで、AI が新規作成する SUB の細分化度と上限を変更する。
...
| 実装言語 | TypeScript |
| データ | IndexedDB、設定のみ `chrome.storage.local` |
